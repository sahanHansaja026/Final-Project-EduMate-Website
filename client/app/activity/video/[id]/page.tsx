...
type VideoData = {
    ...
    title: string;
    description?: string;
    source_type: "Upload" | "External" | string; // Matches your backend Form logic
    video_url: string;
...
export default function ViewVideoPage() {
    ...
    const fetchVideo = async () => {
        try {
            // Adjusted endpoint matching your Python Router GET prefix: @router.get("/{video_id}")
    ...
    const getStatus = () => {
        if (!video?.open_date) return "AVAILABLE"; // If no lock date, treat as always open
    ...
    // Helper to format source paths for files saved locally vs raw URL sources
    const getVideoSrc = () => {
        if (!video) return "";
        if (video.source_type === "Upload") {
            return `${API_BASE_URL}/${video.video_url}`;
        }
        return video.video_url; // External streaming / Embedded URL link
    };
                            ...
                            video.source_type === "Upload" ? (
                                <div className="border border-gray-200 bg-gray-50 p-1 md:p-2 shadow-2xl">
                                    ...
                                    <video
                                        ...
                                        poster={video.thumbnail_url ? `${API_BASE_URL}/${video.thumbnail_url}` : undefined}
                                ...
                                <div className="bg-gray-900 text-white p-12 text-center border border-gray-800">
                                    ...
                                    <h3 className="text-xl mb-4 font-serif">External Media Resource</h3>
