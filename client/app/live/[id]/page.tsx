...
const INITIAL_PARTICIPANTS: Participant[] = [
  { id: "1", name: "You (Host)", role: "host", isMuted: false, isCameraOff: true, isHandRaised: false, avatar: "YH", isActive: true },
...
// ─── Local Video Tile (uses real camera stream) ───────────────────────────────
function LocalVideoTile({
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden bg-[#161622] border-2 border-[#007AFF]/50 aspect-video">
      {/* Video element always rendered so ref is always attached */}
      <video
        ref={videoRef}
        autoPlay
        playsInline
      ...
      <div className="absolute bottom-0 left-0 right-0 px-2.5 py-1.5 bg-gradient-to-t from-black/60 to-transparent flex items-center gap-1.5">
        ...
        <span className="text-xs text-white/90 font-medium truncate flex-1">You (Host)</span>
...
export default function MeetingRoom() {
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        try {
          ...
          // autoPlay attribute handles this in most browsers; ignore NotAllowedError on play()
        }
      }

      setIsCameraOff(false);
      // Update participants list to reflect host camera on
      setParticipants(prev =>
        prev.map(p => p.id === "1" ? { ...p, isCameraOff: false } : p)
      );
    } catch (err: unknown) {
      ...
      if (error.name === "NotFoundError" || error.name === "DevicesNotFoundError") {
        ...
        setMediaError("Camera permission denied. Please allow camera access in your browser settings.");
      } else {
        setMediaError(`Camera error: ${error.message}`);
      }
      setIsCameraOff(true);
    }
  }, []);
  ...
  const stopCamera = useCallback(() => {
    ...
    setIsCameraOff(true);
    setParticipants(prev =>
      prev.map(p => p.id === "1" ? { ...p, isCameraOff: true } : p)
    );
  }, []);
  ...
  }, [isCameraOff, startCamera, stopCamera]);
  ...
  const startMicrophone = useCallback(async () => {
    ...
    try {
      ...
      // Use Web Audio API to control mute state via gain node
      ...
      gainNode.gain.value = 1; // unmuted
      source.connect(gainNode);
      // Note: We do NOT connect to destination to avoid echo — the stream is captured only
      // In a real app you'd send to WebRTC peer connection here

      setIsMuted(false);
      setParticipants(prev =>
        prev.map(p => p.id === "1" ? { ...p, isMuted: false } : p)
      );
    } catch (err: unknown) {
      const error = err as Error;
      if (error.name === "NotFoundError" || error.name === "DevicesNotFoundError") {
        ...
        setMediaError("Microphone permission denied. Please allow microphone access in your browser settings.");
      } else {
        setMediaError(`Microphone error: ${error.message}`);
      }
      setIsMuted(true);
    }
  }, []);
  ...
  const toggleScreenShare = useCallback(async () => {
    if (isScreenSharing) {
      ...
      // In a real app: stop the screen share stream
      return;
    }
    try {
      // getDisplayMedia returns a MediaStream; we just toggle state here
      // In a production app you'd replace the video track in the peer connection
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
          {viewMode === "whiteboard" ? (
            ...
            <div className="flex-1 flex flex-col p-3 gap-3 overflow-hidden">
              {/* Spotlight: local video fills the main area */}
              <div className="flex-1 min-h-0">
                <LocalVideoTile isCameraOff={isCameraOff} isMuted={isMuted} videoRef={videoRef} />
              </div>
              {/* Thumbnails row */}
              <div className="flex gap-2 overflow-x-auto pb-1 flex-shrink-0">
                {participants.filter(p => p.id !== "1").map(p => (
                  <div key={p.id} className="w-28 flex-shrink-0 cursor-pointer"
                    onClick={() => setParticipants(prev => prev.map(pp => ({ ...pp, isActive: pp.id === p.id })))}>
                    <VideoTile p={p} />
                  </div>
                ))}
              </div>
            ...
            // Gallery view — show local video tile for "You" slot, others as mock tiles
            <div className="flex-1 p-3 overflow-auto">
              <div
                className="grid gap-2 h-full"
                style={{ gridTemplateColumns: `repeat(${Math.min(participants.length <= 2 ? 2 : participants.length <= 4 ? 2 : 3, 3)}, 1fr)` }}
              >
                {/* First tile is local video */}
                <LocalVideoTile isCameraOff={isCameraOff} isMuted={isMuted} videoRef={videoRef} />
                {/* Rest are simulated remote participants */}
                {participants.filter(p => p.id !== "1").map(p => (
          ...
          <aside className="w-72 md:w-80 flex flex-col border-l border-white/5 bg-[#0D0D14]/80 flex-shrink-0">
            ...
            {activeTab === "participants" && (
              <div className="flex-1 overflow-y-auto py-2">
                {(["host", "co-host", "participant"] as Participant["role"][]).map(role => {
                ...
                <div className="p-3 border-t border-white/5 flex-shrink-0">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-[#007AFF]/50 focus:bg-white/[0.08] transition-all"
                    />
